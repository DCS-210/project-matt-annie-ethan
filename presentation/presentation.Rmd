...
### Wolrd Bank GDP Per-Capita Dataset
...
facility_df2 <- alldataunfiltered %>% 
  ...
  mutate(count = n()) %>%
  select(disc_facility, disc_facility_country, facility_type, facility_latitude, facility_longitude, count) %>%
  distinct() %>%
  filter(!disc_facility == "Multiple Observatories") %>%
...
labels <- sprintf(
  "<strong>%s</strong><br/>%g Discoveries<br/>Ownership: %s",
  facility_df2$disc_facility, facility_df2$count, facility_df2$disc_facility_country
...
---
### Change in distance of discovery over time by discovery method
```{r animate-scatter, warning = FALSE, echo = FALSE, fig.align = 'center'}
...
---
## Exoplanets' radius
```{r discovery-method-vs-planet_radius, echo = FALSE, fig.show="hold", fig.align = 'center'}
alldata %>%
...
---
## Exoplanets' mass
```{r discovery-method-vs-planet-mass, echo = FALSE, fig.show="hold", fig.align = 'center'}
alldata %>%
  ggplot(mapping = aes(x = disc_year,
                       y = pl_mass,
