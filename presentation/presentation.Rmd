...
## Trends in exoplanet discovery over the course of 20 years
...
- As of today, we have discoverd roughly 4500 exoplanets
...
knitr::include_graphics("https://github.com/DCS-210/project-matt-annie-ethan/blob/main/Ethan.gif")
```

<img src = "/cloud/project/Ethan.gif" width = "40" height = "40"/>
...
```{r gdp-vs-count, echo=FALSE, fig.align = "center"}
...
# Plots
```{r disc-distance-over-time-plot}
...
```{r discovery-method-over-time, warning = FALSE, echo = FALSE, out.width = "50%"}
alldata_discoverymethod <- alldataunfiltered %>%
  ...
  filter(!discoverymethod == "Microlensing")
...
```{r discovery-facility-leaflet-labels}

labels <- sprintf(
  "<strong>%s</strong><br/>%g Discoveries",
  facility_df2$disc_facility, facility_df2$count
) %>% lapply(htmltools::HTML) 

```

```{r discovery-facility-leaflet} 

leaflet(facility_df2) %>% 
      addProviderTiles(providers$Stamen.Terrain) %>%
      addProviderTiles(providers$Stamen.TerrainLabels) %>%
      addCircleMarkers(~facility_longitude, ~facility_latitude, popup = ~labels)
