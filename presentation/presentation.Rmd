...
```{r mapping-facilities, echo = FALSE, out.width = "400%", fig.align = "center"}
...
p <- ggplot() + coord_fixed() +
  xlab("") + ylab("") + labs(subtitle = "Discovery Facilities Mainly Located in Developed Nations") 
...
map_data <- 
  ...
  geom_point(data = facility_df2, 
             ...
             guides(size = "none") +   
             guides(fill = guide_legend("Facility Type"))
...
---
### Exoplanet Discovery Over Time (Linear Fit)


```{r disc-distance-over-time-plot-linear, echo = FALSE, warning = FALSE, message = FALSE, results = 'hide', fig.align='center'}
...
```
---
### How does the mean distance of discovered exoplanets change over time (discovery year)?

```{r count-on-distance-reg-df, include = FALSE, message = FALSE}
...
``` {r distance-on-time-reg, message = FALSE}
...
### How does the number of discovered exoplanets change over time (discovery year)?
``` {r count-on-time-reg}
time_reg <- linear_reg() %>%
  set_engine("lm") %>%
...
### How does the number of discovered exoplanets change based on the mean distance of discovered exoplanets?
...
### How does the number of discovered exoplanets change based on the mean distance of discovered exoplanets over time?
...
- The best model is one that includes both mean distance and discovery year, since a combined model has less omitted variable bias and a higher R-squared value

- The results suggest that as the mean distance in a given year increases by 1 parsec, we can expect an increase in discoveries of approximately 0.452

- The results also suggest that we can expect roughly 12 new discoveries each year on account of time alone (this likely represents technological advancements over time)
...
### GDP and the Number of Exoplanets Discovered (Linear Fit)

```{r gdp-vs-count-linear, echo = FALSE, warning = FALSE, message = FALSE, results = 'hide', fig.align = "center"}
...
---
### What is the relationship between the number of discovered exoplanets and countries GDP in 2020?
...
```
The r-squared value of 0.28 suggests a weak positive correlation between the number of discovered exoplanets and countries GDP in 2020.
---
...
### GDP & the Number of Exoplanets Discovered (Exluding USA) (Linear Fit)
```{r gdp-vs-count-without-USA-linear, echo = FALSE, warning = FALSE, message = FALSE, results = 'hide', fig.align = "center"}
# GDP per capita vs count (No USA)
gdp_country_disc_count_no_USA %>%
  ggplot(mapping = aes(
                        x = GDP_2020,
...
### What is the relationship between the number of discovered exoplanets and countries GDP in 2020? (excluding USA)
...
glance(gdp_disc)$r.squared
```
The r-squared value of 0.0000112 suggests no correlation (or an extremely weak positive correlation) between the number of discovered exoplanets and countries GDP in 2020.
---
...
---
### Multivariable model: The effect of mean distance, discovery year, dominant discovery method, and countries 2020 GDP per-capita on the number of exoplanets discovered

```{r add-count-and-disc_year-to-df, echo = FALSE}
alldata_discoverymethod <- alldata_discoverymethod %>% 
  group_by(disc_year) %>%
  mutate(count = n()) %>%
  mutate(mean_distance = mean(sy_distance))
...
multivariable_model <-
  linear_reg() %>% 
  set_engine("lm") %>%
  fit(count ~ mean_distance + disc_year + discoverymethod + GDP_2020, data = alldata_discoverymethod)

glance(multivariable_model)$r.squared

```
The r-squared is 0.75 which suggests that **75% of the variance in the number of discovered exoplanets can be predicted by the mean distance, discovery year, dominant discovery method, and countries 2020 GDP**. Thus, this multivariable model is much better and more thorough than the simple linear models we presented in the first few slides where only one or two predictor variables (e.g. year/ distance/GDP) were used to predict the outcome variable (count).
...
### Exploring the Data: Interactive Leaflet Map (Discovery Locations)
